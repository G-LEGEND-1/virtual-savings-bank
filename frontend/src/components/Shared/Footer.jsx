      ...
      <div className="flex justify-around">
        {footerItems.map((item) => {
          const isActive = currentPath === item.path || 
                          ...
                          (item.path === '/customer-service' && currentPath === '/support');
          
          return (
            <button
              key={item.id}
              onClick={() => navigate(item.path)}
              className={`flex-1 py-3 flex flex-col items-center ${isActive ? 'text-blue-600' : 'text-gray-500'}`}
            >
              <i className={`${item.icon} text-lg mb-1 ${isActive ? 'text-blue-600' : 'text-gray-400'}`}></i>
